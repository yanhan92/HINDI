...
void OnTick()
{
   ...
   { 
      ...
      SellTicket = OpenSellOrder(Symbol(),LotSize,Slippage,0001);
      ...
      LotSize = VerifyLotSize(LotSize);
      int Slippage = GetSlippage(Symbol(), 5);
      BuyTicket = OpenBuyOrder(Symbol(),LotSize,Slippage,0001);
...
bool CheckNHarami()
{
   ...
   if(Diff1>0.0010 && Diff2<0.0 && Diff1+Diff2< 0.0 && Diff0>=0.0 && Diff2/Diff1<-2)
   {  
       return true;
   }
   else return false;
...
bool CheckPHarami()
{
   ...
   Diff1=iClose(NULL,0,2)-iOpen(NULL,0,2);
   Diff2=iClose(NULL,0,1)-iOpen(NULL,0,1);
   if(Diff1<-0.0010 && Diff2>0.0  &&  Diff1+Diff2> 0.0 && Diff0<=0.0 && Diff2/Diff1>-8 && Diff1 > -0.0060)
   {  
       return true;
   }
   else return false;
